...
?read.csv()
demo_table <- read.csv(file='demo.csv',check.names=F,stringsAsFactors = 
library(jsonlite)
> ?fromJSON()
?fromJSON()
...
plt + geom_tile() + labs(x="Model",y="Vehicle Year",fill="Mean Highway (MPG)") + #add heatmap with labels > theme(axis.text.x = element_text(angle=90,hjust=1,vjust=.5)) #rotate x-axis labels 90 degreesplt <- ggplot(mpg,aes(x=manufacturer,y=hwy)) #import dataset into ggplot2
plt + geom_boxplot() + #add boxplot
